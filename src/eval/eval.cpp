...
namespace stoat::eval {
    ...
    Score correctedStaticEval(
        ...
        const auto correction = corrhist.correction(pos, moves, ply);
        return std::clamp(scaledEval + static_cast<Score>(correction * complexityFactor), -kScoreWin + 1, kScoreWin - 1);
    }
} // namespace stoat::eval
